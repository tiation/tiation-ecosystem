...
# Repository configurations with branch information
declare -A REPO_BRANCHES=(
    ["tiation-terminal-workflows"]="main"
    ["tiation-docker-debian"]="master"
    ["tiation-ai-platform"]="main"
    ["tiation-ai-agents"]="main"
    ["tiation-cms"]="main"
    ["tiation-go-sdk"]="main"
    ["tiation-automation-workspace"]="main"
    ["DiceRollerSimulator"]="master"
    ["liberation-system"]="main"
    ["tiation-chase-white-rabbit-ngo"]="master"
    ["tiation-economic-reform-proposal"]="gh-pages"
    ["tiation-macos-networking-guide"]="main"
    ["tiation-parrot-security-guide-au"]="main"
    ["ubuntu-dev-setup"]="main"
)
...
enable_github_pages() {
    local repo_name=$1
    local branch=${REPO_BRANCHES[$repo_name]}
...
EOF
    ...
    if gh api repos/tiaastor/$repo_name/pages --method POST --input "$temp_file" > /dev/null 2>&1; then
        ...
        # Check if it's already building
        sleep 2
...
test_all_pages() {
    ...
    for repo_name in "${!REPO_BRANCHES[@]}"; do
